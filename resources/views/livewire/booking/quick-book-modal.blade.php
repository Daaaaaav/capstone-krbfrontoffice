        ...
        <div class="relative z-10 w-full max-w-xl bg-card rounded-2xl border border-border shadow-2xl overflow-hidden"
            ...
            <div class="p-6 space-y-4 max-h-[72vh] overflow-y-auto">
                ...
                <div class="flex rounded-xl border border-border overflow-hidden">
                    <button type="button" wire:click="$set('booking_type','meeting')"
                            class="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold transition
                                   {{ $booking_type === 'meeting' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted' }}">
                    ...
                    <button type="button" wire:click="$set('booking_type','online_meeting')"
                            class="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold transition
                                   {{ $booking_type === 'online_meeting' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted' }}">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                  d="M15 10l4.553-2.276A1 1 0 0121 8.723v6.554a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z"/>
                        </svg>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @if ($booking_type === 'meeting')
                        <div>
                            ...
                            <select wire:model.live="room_id"
                                ...
                                @foreach ($rooms as $r)
                                    <option value="{{ $r['id'] }}" @selected($room_id == $r['id'])>{{ $r['name'] }}</option>
                                @endforeach
                            </select>
                    ...
                    @else
                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Platform</label>
                            <div class="flex rounded-lg border border-border overflow-hidden h-10">
                                <button type="button" wire:click="$set('online_provider','google_meet')"
                                        class="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold transition
                                               {{ $online_provider === 'google_meet' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted' }}">
                                ...
                                <button type="button" wire:click="$set('online_provider','zoom')"
                                        class="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold transition
                                               {{ $online_provider === 'zoom' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted' }}">
                                    <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <div>
                        ...
                        <input type="date" wire:model.live="date" value="{{ $date }}"
                               class="w-full h-10 px-3.5 border border-input rounded-lg bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="time" wire:model.live="start_time" value="{{ $start_time }}" min="{{ $minStart }}"
                               class="w-full h-10 px-3.5 border border-input rounded-lg bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                    ...
                    <div>
                        ...
                        <input type="time" wire:model.live="end_time" value="{{ $end_time }}" min="{{ $start_time ?: $minStart }}"
                               class="w-full h-10 px-3.5 border border-input rounded-lg bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" wire:model.live="meeting_title" value="{{ $meeting_title }}"
                    ...
                    <div>
                        ...
                        <input type="number" wire:model.live="number_of_attendees" value="{{ $number_of_attendees }}" min="1" placeholder="0"
                               class="w-full h-10 px-3.5 border border-input rounded-lg bg-background text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                ...
                @if ($booking_type === 'meeting')
                            ...
                            @foreach (['projector', 'whiteboard', 'video_conference', 'catering', 'other'] as $req)
                                <label class="flex items-center space-x-2.5 cursor-pointer group">
                                    <input type="checkbox" wire:model.live="requirements" value="{{ $req }}"
                                           @checked(in_array($req, $requirements ?? []))
                    ...
                    @if (in_array('other', $requirements ?? [], true))
                        <div>
                            ...
                            <textarea wire:model.live="special_notes" rows="3" placeholder="{{ __('app.quick_book_notes_ph') }}"
                                      class="w-full px-3.5 py-2.5 border border-input rounded-lg bg-background text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none">{{ $special_notes }}</textarea>
