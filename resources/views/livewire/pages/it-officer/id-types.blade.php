                    ...
                    <tbody class="divide-y divide-[#d4dfc8]">
                        @forelse($idTypes as $idType)
                            <tr class="hover:bg-[#f0f4eb] transition">
                                <td class="px-6 py-4 text-[#9aaa8a] text-xs">{{ $idTypes->firstItem() + $loop->index }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-medium text-[#2d3a24]">{{ $idType->id_type_name }}</span>
                                    </div>
                                </td>

                                {{-- USAGE COUNT --}}
                                <td class="px-6 py-4">
                                    @if($idType->guestbooks_count > 0)
                                ...
                                <td class="px-6 py-4 text-[#5a6e4a] text-xs">
                                    {{ $idType->created_at->format('d M Y') }}
                                </td>
                ...
                <form wire:submit.prevent="save" class="flex flex-col overflow-hidden">
                    <div class="p-6 space-y-4 overflow-y-auto">
                        ...
                        <div>
                            ...
                            @if($editMode)
                                @error('edit_id_type_name') <p class="text-xs text-destructive mt-1.5 font-medium">{{ $message }}</p> @enderror
                            @else
                                @error('id_type_name') <p class="text-xs text-destructive mt-1.5 font-medium">{{ $message }}</p> @enderror
                            @endif
                        </div>
