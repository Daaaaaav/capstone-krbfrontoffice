    ...
    @if($showBookingDetailModal && !empty($selectedBookingDetail))
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6" x-data="{ showDetail: @entangle('showBookingDetailModal') }" x-show="showDetail" x-cloak>
            ...
            <div class="relative w-full max-w-lg bg-card border border-border rounded-2xl shadow-2xl overflow-hidden transform transition-all flex flex-col"
                ...
                <div class="px-6 py-4 border-b border-border {{ !empty($selectedBookingDetail['is_priority']) ? 'bg-amber-50/60' : 'bg-muted/5' }} flex items-start justify-between">
                    <div>
                        <div class="flex items-center gap-2">
                            @if(!empty($selectedBookingDetail['is_priority']))
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold tracking-wide uppercase
                                {{ !empty($selectedBookingDetail['is_priority']) ? 'bg-amber-500/15 text-amber-700' : 'bg-primary/10 text-primary' }}">
                                {{ $selectedBookingDetail['status'] }}
                            </span>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">
                                {{ !empty($selectedBookingDetail['is_priority']) ? 'Requested by (Manager)' : 'Borrower' }}
                            </p>
                            <p class="text-sm font-semibold text-foreground">
                                {{ !empty($selectedBookingDetail['is_priority']) ? ($selectedBookingDetail['requested_by'] ?? '—') : $selectedBookingDetail['borrower'] }}
                            </p>
                        </div>
                        @if(empty($selectedBookingDetail['is_priority']))
                        <div>
                        ...
                        @if(!empty($selectedBookingDetail['attendees']))
                        <div>
                            <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Attendees</p>
                            <p class="text-sm font-semibold text-foreground">{{ $selectedBookingDetail['attendees'] }}</p>
                        </div>
                        @endif
                        <div>
