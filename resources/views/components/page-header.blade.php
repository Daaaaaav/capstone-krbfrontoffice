...
<div class="relative overflow-hidden rounded-2xl bg-[#4A2F24] shadow-2xl">
    ...
    <div class="relative z-10 px-6 sm:px-8 py-5 sm:py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-lg sm:text-xl font-semibold tracking-tight" style="color:#fde047">{{ $title }}</h1>
                @if ($subtitle)
                    <p class="text-sm mt-0.5" style="color:rgba(253,224,71,0.75)">{{ $subtitle }}</p>
                @endif
            </div>
            @if (isset($actions))
