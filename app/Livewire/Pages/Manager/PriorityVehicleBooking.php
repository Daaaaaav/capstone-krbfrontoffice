...
class PriorityVehicleBooking extends Component
{
    ...
    // Mark Done modal for Priority Vehicle Bookings
    public bool $showMarkDoneModal = false;
    public ?int $markDoneId = null;
    public ?string $markDonePhotoData = null;

    public function openMarkDoneModal(int $id): void
    {
        $this->markDoneId = $id;
        $this->markDonePhotoData = null;
        $this->showMarkDoneModal = true;
    }

    public function closeMarkDoneModal(): void
    {
        $this->showMarkDoneModal = false;
        $this->markDoneId = null;
        $this->markDonePhotoData = null;
    }

    public function submitMarkDone(): void
    {
        \App\Services\SecurityMonitoringService::logFormSubmit(class_basename($this), method_exists($this, 'all') ? $this->all() : []);

        $this->validate([
            'markDoneId' => 'required|integer',
            'markDonePhotoData' => 'nullable|string',
        ]);
        ...
        try {
            DB::transaction(function () use ($user, $companyId) {
                $booking = PriorityVehicleBookingModel::lockForUpdate()
                    ->where('id', $this->markDoneId)
                    ->where('company_id', $companyId)
                    ->where('manager_id', $user->user_id)
                    ->firstOrFail();

                if ($booking->status !== PriorityVehicleBookingModel::STATUS_ON_PROGRESS) {
                    throw new \RuntimeException('Only bookings with "On Progress" status can be marked as done.');
                }

                // Save photo if provided
                if ($this->markDonePhotoData) {
                    $booking->return_photo = \App\Services\ImageHelper::storeBase64AsWebp(
                        $this->markDonePhotoData,
                        'vehicle_evidences',
                ...
                ManagerNotification::notifyReceptionists(
                    $companyId,
                    ManagerNotification::TYPE_PRIORITY_VEHICLE_DIRECT,
                    'Priority Vehicle Booking Completed',
                    'Manager "' . ($user->full_name ?? $user->name) . '" marked priority vehicle booking for "' .
                        $vehicleLabel . '" as completed.',
                    $booking,
                    actionRequired: false
                );
            });

            $this->closeMarkDoneModal();
            $this->resetPage();
            $this->dispatch('toast', type: 'success', title: 'Completed', message: 'Priority vehicle booking marked as done.', duration: 3500);
        } catch (\RuntimeException $e) {
            $this->dispatch('toast', type: 'warning', title: 'Cannot Complete', message: $e->getMessage(), duration: 4000);
        } catch (\Throwable $e) {
            report($e);
            $this->dispatch('toast', type: 'error', title: 'Error', message: 'Failed to mark as done: ' . $e->getMessage(), duration: 4000);
        }
    }
