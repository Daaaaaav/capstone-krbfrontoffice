...
enum ChatDataSource: string
{
    ...
    public function label(?string $customName = null): string
    {
        return match ($this) {
            ...
            self::APPROVED_EXTERNAL_DATASET => $customName ? "Approved KRB Data Source: {$customName}" : 'Approved KRB Data Source',
    ...
    public function description(): string
    {
        return match ($this) {
            ...
            self::APPROVED_EXTERNAL_DATASET => 'Retrieved from explicitly approved external dataset',
            self::COMBINED                  => 'Combined metrics from live application records and historical CSV data',
        };
    }
    ...
    public static function formatSourcesTag(array $sources): string
    {
        ...
        foreach ($sources as $source) {
            if (is_string($source)) {
                ...
                if ($enum) {
                    $custom = $source['source_name'] ?? $source['file'] ?? null;
